...
public class ConnectionPoolImplTest {
    ...
    private static class TestConnection implements Connection<TestClient> {
        ...
        @Override
        public <R> ListenableFuture<OperationResult<R>> executeAsync(AsyncOperation<TestClient, R> op)
    ...
    private static ConnectionFactory<TestClient> connFactory = new ConnectionFactory<TestClient>() {
        ...
        @Override
        public Connection<TestClient> createConnection(HostConnectionPool<TestClient> pool, ConnectionObservor observor)
    ...
    public void testConnectionPoolNormal() throws Exception {
        ...
        final ConnectionPoolImpl<TestClient> pool = new ConnectionPoolImpl<TestClient>(connFactory, cpConfig,
                cpMonitor);
    ...
    private TokenMapSupplier getTokenMapSupplier() {
        ...
        /**
         * cqlsh:dyno_bootstrap> select "availabilityZone","hostname","token"
         * from tokens where "appId" = 'dynomite_redis_puneet';
         * 
    ...
    public void testAddingNewHosts() throws Exception {
        ...
        Assert.assertTrue("h3Stats: " + h3Stats + " h1Stats: " + h1Stats,
                h1Stats.getOperationSuccessCount() > h3Stats.getOperationSuccessCount());
        Assert.assertTrue("h3Stats: " + h3Stats + " h2Stats: " + h2Stats,
                h2Stats.getOperationSuccessCount() > h3Stats.getOperationSuccessCount());
    }
    ...
    public void testRemovingHosts() throws Exception {
        ...
        Assert.assertTrue("h1Stats: " + h1Stats + " h2Stats: " + h2Stats,
                h1Stats.getOperationSuccessCount() > h2Stats.getOperationSuccessCount());
        Assert.assertTrue("h2Stats: " + h2Stats + " h3Stats: " + h3Stats,
                h3Stats.getOperationSuccessCount() > h2Stats.getOperationSuccessCount());
    }
    ...
    public void testIdleWhenNoAvailableHosts() throws Exception {
        ...
        try {
            ...
            // calling idle() again should have no effect and will throw an
            // exception if the pool has already been
            // started
            pool.idle();
        } finally {
            pool.shutdown();
    ...
    public void testPoolTimeout() throws Exception {
        ...
        // Now exhaust all 9 connections, so that the 10th one can fail with
        ...
        final Callable<Void> blockConnectionForSomeTime = new Callable<Void>() {
            ...
            public Void call() throws Exception {
                try {
                    Thread.sleep(10000); // sleep for a VERY long time to ensure
                                         // pool exhaustion
        ...
        latch.await();
        Thread.sleep(100); // wait patiently for all threads to have blocked the
                           // connections
    ...
    public void testPoolOffline() {
        ...
        // The test logic is simply to throw an exception so that errors will
        // recorded by the health tracker
    ...
    public void testHostEvictionDueToErrorRates() throws Exception {
        ...
        final ConnectionFactory<TestClient> badConnectionFactory = new ConnectionFactory<TestClient>() {
            ...
            @Override
            public Connection<TestClient> createConnection(final HostConnectionPool<TestClient> pool,
                    ConnectionObservor cObservor) throws DynoConnectException, ThrottledException {
        ...
        final ConnectionPoolImpl<TestClient> pool = new ConnectionPoolImpl<TestClient>(badConnectionFactory, cpConfig,
                cpMonitor);
        hostSupplierHosts.add(host1);
        hostSupplierHosts.add(host2);
        hostSupplierHosts.add(host3);
        ...
        Assert.assertTrue("Total ops: " + client.ops.get(), client.ops.get() > 0);
        Assert.assertTrue("Total errors: " + cpMonitor.getOperationFailureCount(),
                cpMonitor.getOperationFailureCount() > 0);
        ...
        Assert.assertEquals(client.ops.get() + cpMonitor.getOperationFailureCount(),
                cpMonitor.getConnectionBorrowedCount());
        Assert.assertEquals(client.ops.get() + cpMonitor.getOperationFailureCount(),
                cpMonitor.getConnectionReturnedCount());
    ...
    public void testWithRetries() throws Exception {
        ...
        final ConnectionPoolImpl<TestClient> pool = new ConnectionPoolImpl<TestClient>(badConnectionFactory,
                cpConfig.setRetryPolicyFactory(rFactory), cpMonitor);
        hostSupplierHosts.add(host1);
    ...
    private void executeTestClientOperation(final ConnectionPoolImpl<TestClient> pool,
            final Callable<Void> customLogic) {
    ...
    private void runTest(final ConnectionPoolImpl<TestClient> pool, final Callable<Void> customTestLogic)
            throws Exception {
                        ...
                        while (!stop.get() && !Thread.currentThread().isInterrupted()) {
                            try {
                                pool.executeWithFailover(new Operation<TestClient, Integer>() {
                                    ...
                                    @Override
                                    public Integer execute(TestClient client, ConnectionContext state)
                                            throws DynoException {
                                        client.ops.incrementAndGet();
                                        return 1;
                                    }
                                ...
                                // System.out.println("FAILED Test Worker
